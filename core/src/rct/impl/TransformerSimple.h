...
class TransformerSimple: public TransformerCore {
public:
	typedef boost::shared_ptr<TransformerSimple> Ptr;
	typedef rsc::threading::Future<Transform> FutureType;
	typedef boost::shared_ptr<FutureType> FuturePtr;

	TransformerSimple(const boost::posix_time::time_duration& cacheTime);
	virtual ~TransformerSimple();
...
private:
	...
	public:
		...
		Request(const std::string& target_frame, const std::string& source_frame, const boost::posix_time::ptime& time) :
